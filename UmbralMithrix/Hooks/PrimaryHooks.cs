...
namespace UmbralMithrix
{
    public class PrimaryHooks
    {
        ...
        {
            if (self.isAuthority && self.hasDoneBlastAttack && self.modelTransform && !UmbralMithrix.hasfired)
            {
        ...
        {
            if (self is not FireLunarShardsHurt)
            {
                ...
                for (int index = 0; index < ModConfig.LunarShardAdd.Value; ++index)
                {
                    ...
                    aimRay.direction = Util.ApplySpread(aimRay.direction, 0.0f, self.maxSpread * (float)(1.0 + 0.449999988079071 * index), self.spreadYawScale * (float)(1.0 + 0.449999988079071 * index), self.spreadPitchScale * (float)(1.0 + 0.449999988079071 * index));
                    fireProjectileInfo.rotation = Quaternion.LookRotation(aimRay.direction);
                }
            }
